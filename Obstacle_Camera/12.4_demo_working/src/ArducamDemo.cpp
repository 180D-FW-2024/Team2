...
bool detectPurple(const cv::Mat& image) {
    cv::Mat hsvImage;
    cv::cvtColor(image, hsvImage, cv::COLOR_BGR2HSV);

    cv::Scalar lowerPurple(130, 50, 50);
    cv::Scalar upperPurple(160, 255, 255);

    cv::Mat purpleMask;
    cv::inRange(hsvImage, lowerPurple, upperPurple, purpleMask);

    int purplePixels = cv::countNonZero(purpleMask);
    return purplePixels > 100; // Adjust threshold as needed
}
