...
def vibrate_all_motors(pins, duration=0.2):
    print("Vibrating all motors")
    set_pins_high(pins)
    time.sleep(duration)
    set_pins_low(pins)
...
def camera_monitor(process, pins, shared_state):
...
def monitor_and_control():
    ...
    try:
        ...
        camera_thread = threading.Thread(target=camera_monitor, 
                                       args=(process, pins, shared_state))
        camera_thread.daemon = True
        camera_thread.start()

        print("\n=== Keyboard Control Mode Active ===")
        while True:
            if not shared_state['obstacle_mode']:
