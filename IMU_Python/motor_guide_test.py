...
while True:
    ...
    if (left):
        if (not angle_changed):
            expected_angle = expected_angle + 90
            angle_changed = True
    if (right):
            if (not angle_changed):
                expected_angle = expected_angle - 90
                angle_changed = True
    if (1):
        difference = abs(gyroZangle-expected_angle) 
        ...
        if (difference > 25):
            print("NEED TO TURN")
            M3.off()
            if (difference > 80 and right):
                M5.on()
                time.sleep(.1)
                M5.off()
            else:
                M5.off()
            if (difference > 80 and left): 
                M1.on()
                time.sleep(.1)
                M1.off()
            else:
                M1.off()
            if (25 < difference <= 80 and right):
                M4.on()
                time.sleep(.1)
                M4.off()
            else:
                M4.off()
            if (-25 > difference >= -80 and left):
                M2.on()
                time.sleep(.1)
                M2.off()
            else:
