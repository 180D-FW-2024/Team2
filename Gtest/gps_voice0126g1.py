...
def narrate_full_route(route_steps):
...
def navigate_route(route_steps):
    ...
    for step in route_steps:
        ...
        while True:
            ...
            if distance_to_next < 10:  # Threshold for reaching the next step
                print("SoleMate: Step completed. Proceeding to the next instruction.")
                break

            if time.time() - start_time > 30:  # Timeout for no progress
    ...
    while True:
        ...
        if destination_input:
            ...
            if place_info:
                ...
                if route_steps:
                    narrate_full_route(route_steps)  # Narrate full route before starting
