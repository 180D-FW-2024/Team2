...
import json
print(json.dumps(directions, indent=4))  # Pretty print the response

# Initialize Text-to-Speech engine
tts = pyttsx3.init()
tts.setProperty("rate", 150)  # Adjust speech rate

# Step 1: Connect to gpsd and get current location
try:
    gpsd.connect()
    ...
    tts.say(f"Error connecting to GPS")
    tts.runAndWait()
    exit(1)
...
try:
    ...
    steps = directions[0]["legs"][0].get("steps", [])  # Ensure steps exist
