...
bool detectObjectWithin20(const cv::Mat& image) {
    ...
    cv::addWeighted(gradX, 0.5, gradY, 0.5, 0, gradMagnitude);
    
    // Apply threshold to get strong edges
    cv::Mat edges;
    cv::threshold(gradMagnitude, edges, 50, 255, cv::THRESH_BINARY);
    
    // Count edge pixels in the lower third of the image
    // Objects closer to the camera will appear larger in the lower portion
    int height = edges.rows;
    cv::Mat lowerRegion = edges(cv::Rect(0, 2*height/3, edges.cols, height/3));
    int edgePixels = cv::countNonZero(lowerRegion);
    
    // Calibrated threshold for 20cm distance
    return edgePixels > (lowerRegion.rows * lowerRegion.cols * 0.12);
}


int main(int argc, char *argv[]){
   printf("Arducam begin.\r\n");
   printf("I added something\r\n"); 
...
while(1){
    ...
    if(rawImage.empty()) {
        ...
        if (detectObjectWithin20(rawImage)) {
            printf("close objects detected in Camera\n");
        }else{
	    printf("nothing detected, good to go!\n");
	}
    }
#endif
#if defined (CAMERA_2_EN)
