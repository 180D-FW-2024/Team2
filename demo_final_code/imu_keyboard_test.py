...
while True:
    ...
    def buzz_motor_for_direction(angle_diff):
        ...
        if 120 <= angle_diff < 240:   # Walk Back
            M6.on()
        elif 240 <= angle_diff < 300:  # Sharp Left
            M1.on()
        elif 300 <= angle_diff < 345:   # Slight Left
            M2.on()
        elif 345 <= angle_diff or angle_diff < 20:   # Forward
            M3.on()
    ...
    if ((abs(gyroXangle-gyroX_avg) > 150) or (abs(gyroYangle-gyroY_avg) > 150)):
       fallen_counter+=1
       if fallen_counter < 3:
